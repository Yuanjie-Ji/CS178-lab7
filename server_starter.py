...
def getConfusionMatrix(prediction_data):
    ...
    for keys in prediction_data:
        ...
        #print([prediction == label for prediction, label in zip(prediction_data[keys], ground_truth) if label == 1])
        ...
        #if keys != "ground_truth": print("{}: {:.2f}".format(keys, ((truePositive+trueNegative)/(falsePositive+falseNegative+truePositive+trueNegative))))
